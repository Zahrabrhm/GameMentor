...
def analyze_gameplay(data_dir="data", threshold=5):
    ...
    with open(data_path / "positions.pkl", "rb") as f:
    ...
    for i in range(len(action_values)):
        ...
        if value_range > threshold:
            important_states.append(i)
            
            if agent_actions[i] == human_actions[i]:
                print(f"State {i} (pos={positions[i]}): Human played correctly")
                correct_actions.append(i)
            else:
                print(f"State {i} (pos={positions[i]}): Human made a mistake")
                mistakes.append(i)
